...
def img_paths():
    ...
    for r, d, fs in os.walk(os.getcwd() + "/config/images"):
        for f in fs:
            ...
            image = cv2.imread(os.getcwd()+"/config/images/"+f)
            new_image = RotateImage(image, 90)
            ...
            x = requests.post("https://freeimage.host/api/1/upload", payload, files={"source": bytes_encoded})
            import json
